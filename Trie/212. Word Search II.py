...
Constructing the trie: ( O(m) ), where ( m ) is the total number of characters in the dictionary words.
DFS traversal: O( n.(4^k) ), where ( n ) is the total number of characters in the board and ( k ) is the maximum length of a word. In the worst case, all cells are part of the word, and there are four recursive calls at each step.

Overall time complexity: O( m+n.(4^k) )


Space complexity:

Storing the trie: ( O(m) ), where ( m ) is the total number of characters in the dictionary words.
Recursive call stack during DFS: ( O(n) ), where ( n ) is the total number of characters in the board.
